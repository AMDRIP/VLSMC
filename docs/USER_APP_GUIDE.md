...
### 2.3. Доступные функции стандартной библиотеки (Libc)
ОС предоставляет изолированную реализацию `libc` для пользовательских процессов (Ring 3). Подключение заголовочных файлов осуществляется напрямую (`#include "libc/include/string.h"`).
- **Память (malloc.h)**: `malloc`, `free`, `calloc`, `realloc`. Менеджер осуществляет автоматическое слияние (`coalescing`) блоков для предотвращения фрагментации и управляет границами кучи поверх вызовов `sys_sbrk`.
- **Примитивы памяти (string.h)**: `memcpy`, `memmove`, `memset`, `memcmp`. Безопасно работают с перекрывающимися областями (overlap).
- **Работа со строками (string.h)**: `strlen`, `strnlen`, `strcpy`, `strlcpy`, `strcmp`, `strncmp`, `strchr`, `strstr`. Реализована строгая проверка границ для минимизации уязвимостей (функции с суффиксом `l` или `n`).
