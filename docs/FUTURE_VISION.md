...
- **AHCI / SATA:** FAT16 работает на эмулируемом IDE (ATA PIO). Переход на AHCI/DMA значительно ускорит чтение файлов и позволит работать на современном железе.
    - *Технические требования:*
        1. **PCI Enumeration**: Поиск контроллера по Class ID 0x01 (Storage), Subclass 0x06 (SATA) и Programming Interface 0x01 (AHCI).
        2. **HBA Memory Mapping**: Маппинг AHCI Base Address Register (BAR5) в виртуальную память ядра.
        3. **GHC Setup**: Инициализация Global Host Control (активация AHCI режима, сброс контроллера).
        4. **Port Rebase**: Настройка структур данных порта (Command List, FIS Receive Area) в физической памяти.
        5. **Command Slot Management**: Реализация очереди команд (32 слота) для асинхронных операций.
        6. **PRDT (Physical Region Descriptor Table)**: Настройка таблицы дескрипторов для DMA-передач, обеспечивающей перенос данных между диском и памятью без участия CPU.
        7. **Interrupt handling**: Поддержка контроллера через MSI или стандартный IRQ (чтение Port Interrupt Status).
        8. **Command Completion**: Реализация механизма завершения команд (completion queue) для асинхронного управления операциями.
- 
