...
### Этап 3: Расширение и Стабилизация

#### 1. Потокобезопасность (Thread Safety)
Поскольку VLSMC поддерживает многопоточность (kernel threads), стандартная библиотека должна быть reentrant.

*   **Malloc/Free Locking**:
    *   Внедрение глобального `spinlock_t malloc_lock` в `malloc.cpp`.
    *   Использование атомарных инструкций `lock bts` / `lock btr` или `xchg` для захвата блокировки перед манипуляцией списком свободных блоков.
    *   *В будущем*: Переход на fine-grained locking (блокировка отдельных арен) или lock-free списки.
*   **Stdio Locking**:
    *   Структура `FILE` расширяется полем `mutex_t lock`.
    *   Реализация `flockfile(FILE*)` и `funlockfile(FILE*)`.
    *   Функции `printf`, `fwrite` и др. обязаны захватывать лок перед записью в буфер, чтобы вывод разных потоков не перемешивался.
*   **Errno**:
    *   Переход от глобальной переменной к макросу `#define errno (*__errno_location())`.
    *   Функция `__errno_location()` возвращает адрес переменной в TLS текущего потока.

#### 2. Защита стека (Stack Smashing Protection)
Компилятор с флагом `-fstack-protector-all` вставляет "канарейку" (canary) в стек функций.

*   **`__stack_chk_guard`**:
    *   Глобальная переменная `uintptr_t` в `.data` секции libc.so.
    *   **Инициализация**: При старте процесса ядро передает вектор `auxv`. `ldso` или `__libc_start_main` должны извлечь значение `AT_RANDOM` и записать его в `__stack_chk_guard`.
*   **`__stack_chk_fail`**:
    *   Функция, вызываемая при несовпадении канарейки.
    *   **Действие**:
        1.  Вывод сообщения "Stack smashing detected" в stderr (через прямой сисколл `write`, минуя stdio, так как куча/стек могут быть повреждены).
        2.  Вызов `sys_abort()` или `sys_exit(127)`.

#### 3. Инициализация и Жизненный цикл (Runtime Start)
Стандартная точка входа `_start` в `crt0.o` заменяется на более сложную цепочку для поддержки динамической линковки.

*   **`__libc_start_main`**:
    *   Сигнатура: `int __libc_start_main(int (*main)(int, char**, char**), int argc, char** ubp_av, void (*init)(), void (*fini)(), void (*rtld_fini)(), void* stack_end)`.
    *   **Задачи**:
        1.  Сохранение аргументов в глобальные `__environ`, `__progname`.
        2.  Инициализация подсистемы защиты стека.
        3.  Инициализация TLS (если статический TLS).
        4.  Вызов `__libc_csu_init` (выполнение `.init_array` всех загруженных библиотек).
        5.  Вызов `main()`.
        6.  Вызов `exit(result)`.
*   **Конструкторы Libc**:
    *   Сама `libc.so` может иметь функции с атрибутом `__attribute__((constructor))`, например, для инициализации malloc-арен. Они помещаются в `.init_array` самой библиотеки.

#### 4. Оптимизация и Совместимость

*   **Lazy Binding (PLT)**:
    *   По умолчанию `ldso` резолвит все символы при старте (`LD_BIND_NOW`).
    *   Для ускорения запуска: `ldso` заполняет GOT-слоты адресом PLT-стаба.
    *   При первом вызове функции управление попадает в `_dl_runtime_resolve`, который находит реальный адрес, пишет его в GOT и передает управление.
*   **TLS (Thread Local Storage)**:
    *   Поддержка ELF-сегмента `PT_TLS`.
    *   Ядро должно поддерживать сегментные регистры `FS` или `GS` для пользовательских потоков.
    *   Системный вызов `sys_set_thread_area` (аналог Linux `set_thread_area` или `arch_prctl`) для настройки базы сегмента.
...
## 9. Чек-лист проверки (QA)

### 9.1. Сборка и Структура
- [ ] Файл `libc.so` распознается как ELF 32-bit LSB shared object (`readelf -h libc.so`).
- [ ] Присутствуют секции `.dynsym`, `.dynstr`, `.hash` (`readelf -S libc.so`).
- [ ] Нет неопределенных символов (UND), кроме тех, что экспортирует ядро или ldso.

### 9.2. Загрузка и Линковка
- [x] `ldso` успешно находит `libc.so` в `/lib/`.
- [x] `ldso` корректно маппит сегменты `LOAD` по адресам, отличным от 0.
- [x] Релокации `R_386_RELATIVE` обработаны (глобальные переменные внутри libc указывают на правильные адреса).
- [x] Релокации `R_386_GLOB_DAT` и `R_386_JMP_SLOT` разрешены.

### 9.3. Функциональность
- [x] **Hello World**: `printf` выводит строку в консоль.
- [x] **Memory**: `malloc` выделяет память, `free` освобождает.
- [ ] **Global State**: Переменная `errno` меняет значение при ошибке сисколла.
- [ ] **Constructors**: Глобальные конструкторы выполняются при загрузке библиотеки.

### 9.4. Потокобезопасность (Thread Safety)
- [x] `malloc`/`free` используют спинлоки для предотвращения гонок.
- [x] `FILE` структуры защищены при конкурентном доступе.
- [x] `errno` использует TLS (если поддерживается ядром).

### 9.5. Защита от переполнения (Stack Smashing Protection)
- [ ] `__stack_chk_guard` экспортируется и инициализируется.
- [ ] `__stack_chk_fail` реализован для безопасного завершения.

### 9.6. Инициализация (Ctors/Dtors)
- [x] `.init_array` и `.fini_array` обрабатываются.
- [x] `__libc_start_main` корректно настраивает окружение.

### 9.7. Оптимизация
- [ ] Lazy Binding работает через PLT/GOT.
- [ ] `DT_RELR` поддерживается (если используется).
